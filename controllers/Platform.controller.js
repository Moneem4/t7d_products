...
exports.updatePlatform=(req,res)=>{
     if (validator(req.body, ["_id"], res)) { return }
        ...
        PlatformModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body._id) }, { $set: params }, { new: true }).exec()
            .then(data => {
            if (data == null) {
                display_costume_error(res, "Platform id not found", 404); return
            } else {
                res.status(res.statusCode).json({
...
exports.deletePlatform=(req,res)=>{
    if (validator(req.body, ["_id"], res)) { return }
    PlatformModel.remove({ _id: mongoose.Types.ObjectId(req.body._id) }).exec().then(data => {
        if (data.deletedCount == 0) {
            display_costume_error(res, "Platform _id not found", 404); return
        } else {
            res.status(res.statusCode).json({
...
exports.getAllPlatform=(req,res)=>{
    PlatformModel.find({}).exec().then(data => {
        res.status(res.statusCode).json({
         data,
    })
    }).catch(error => {
